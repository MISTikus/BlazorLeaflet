...
window.leafletBlazor = {
  create: function (map, objectReference) {
    var leafletMap = L.map(map.id, {
      ...
      maxBounds:
        map.maxBounds && map.maxBounds.item1 && map.maxBounds.item2
  ...
  addTilelayer: function (mapId, tileLayer, objectReference) {
    const layer = L.tileLayer(tileLayer.urlTemplate, {
      ...
      tileSize: tileLayer.tileSize
        ? L.point(tileLayer.tileSize.width, tileLayer.tileSize.height)
        : undefined,
      ...
      bounds:
        tileLayer.bounds && tileLayer.bounds.item1 && tileLayer.bounds.item2
  ...
  addRectangle: function (mapId, rectangle, objectReference) {
    const layer = L.rectangle(
      [
        [rectangle.shape.bottom, rectangle.shape.left],
        [rectangle.shape.top, rectangle.shape.right]
      ],
  ...
  observeMapResize(mapId) {
    ...
    observers[mapId] = new ResizeObserver(() =>
...
function createIcon(icon) {
  return L.icon({
    ...
    shadowSizeAnchor: icon.shadowSizeAnchor
      ? L.point(icon.shadowSizeAnchor.width, icon.shadowSizeAnchor.height)
      : null,
